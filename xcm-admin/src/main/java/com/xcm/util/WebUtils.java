...
public final class WebUtils {
	 ...
	 * 判断是不是ajax请求
	 */
	private final static String AJAX_HEADER_NAME = "X-Requested-With";
	...
	public static void addCookie(HttpServletRequest request, HttpServletResponse response, String name, String value, Integer maxAge, String path, String domain, Boolean secure) {
		...
		try {
			...
			LOGGER.error("添加cookie异常,error catch:", e);
		}
	}
	...
	public static String getCookie(HttpServletRequest request, String name) {
		...
		if (cookies != null) {
			try {
				...
				LOGGER.error("获取cookie异常,error catch:", e);
			}
		}
		return null;
	}
	...
	public static void removeCookie(HttpServletRequest request, HttpServletResponse response, String name, String path, String domain) {
		...
		try {
			...
			LOGGER.error("移除cookie异常,error catch:", e);
		}
	}
	 ...
	 * @param response
	 */
	public static void addLoginCookies(User user, HttpServletRequest request, HttpServletResponse response) {
		WebUtils.addCookie(request, response, User.USER_COOKIE_ID, user.getId().toString(), 365 * 24 * 60 * 60);
	}
	 ...
	 * @param response
	 */
	public static void removeLogoutCookie(HttpServletRequest request, HttpServletResponse response) {
		WebUtils.removeCookie(request, response, "test-cookie");
	}
